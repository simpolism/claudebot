 ...
 */
function markThreadReset(threadId: string, botId: string | null, resetMessageId?: string): void {
 ...
 */
function getThreadResetMessageId(threadId: string, botId: string): string | null {
  const botResets = resetThreads.get(threadId);
...
async function fetchTextAttachment(attachment: Attachment): Promise<string | null> {
  if (!attachment.url) {
    return null;
  }
  ...
  if (attachment.size && attachment.size > MAX_TEXT_ATTACHMENT_BYTES) {
    console.warn(
      `[Attachment] Skipping ${attachment.name ?? attachment.id} - ${attachment.size} bytes exceeds ${MAX_TEXT_ATTACHMENT_BYTES} byte limit`,
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[Attachment] Failed to fetch ${attachment.name ?? attachment.id} - HTTP ${response.status}`,
      );
      return null;
    }
    ...
    if (
      ...
      console.warn(
        `[Attachment] Skipping ${attachment.name ?? attachment.id} - content-length ${contentLengthHeader} exceeds limit`,
      );
      return null;
    }
    ...
    if (buffer.byteLength > MAX_TEXT_ATTACHMENT_BYTES) {
      console.warn(
        `[Attachment] Skipping ${attachment.name ?? attachment.id} - downloaded ${buffer.byteLength} bytes exceeds limit`,
      );
      return null;
    }
    ...
    console.warn(
      `[Attachment] Error fetching ${attachment.name ?? attachment.id}, skipping text inline`,
      err,
    );
    return null;
  }
...
async function getTextAttachmentSections(message: Message): Promise<string[]> {
...
async function messageToStored(message: Message): Promise<StoredMessage> {
...
export function getContext(
  ...
  if (isThreadContext && isResetThread) {
    ...
    if (resetMessageId) {
      ...
      if (resetIndex !== -1) {
        ...
        console.log(
          `[getContext] Thread ${threadId} reset for bot ${botUserId} - filtered to ${currentMessages.length} messages after ${resetMessageId}`,
        );
      }
    }
  }
  ...
  const threadStartTimestamp =
    isThreadContext && currentMessages.length > 0 ? currentMessages[0]!.timestamp : null;
  const snapshotParentMessages =
    isThreadContext && !isResetThread && threadStartTimestamp != null
  ...
  const snapshotParentBoundaries =
    isThreadContext && !isResetThread && threadStartTimestamp != null
      ? currentBoundaries.filter(
          (boundary) =>
            snapshotParentMessageIds.has(boundary.firstMessageId) &&
...
function hydrateChannelFromDatabase(channelId: string): {
  messageCount: number;
  ...
  for (const channelId of channelIds) {
    try {
      ...
      if (currentMessages.length === 0) {
        console.log(
          `[loadHistoryFromDiscord] Channel ${channelId} empty in DB, bootstrapping from Discord history`,
        );
        ...
        if (lastMessage) {
          backfilledCount = await backfillChannelFromDiscord(
            channel,
            channelId,
            lastMessage.id,
          );
      ...
      console.log(
        `[loadHistoryFromDiscord] ${channelId}: hydrated ${hydration.messageCount} DB messages, ` +
  ...
  try {
    ...
    if (resetInfo) {
      if (resetInfo.lastResetDiscordMessageId) {
        console.log(
          `[LazyLoad] Thread ${threadId} was reset for bot ${botUserId} at Discord msg ${resetInfo.lastResetDiscordMessageId}`,
        );
      ...
      if (resetInfo.lastResetRowId) {
        ...
        dbMessages = dbMessages.filter((msg) => {
          if (typeof msg.rowId !== 'number') return true;
          return msg.rowId > resetInfo.lastResetRowId;
        });
        if (dbMessages.length !== filteredCount) {
          console.log(
            `[LazyLoad] Filtered ${filteredCount - dbMessages.length} pre-reset messages from DB for thread ${threadId}`,
          );
        }
      }
    }
    ...
    console.log(
      `[LazyLoad] Loaded ${dbMessages.length} messages from DB, merged with ${newInMemoryMessages.length} in-memory messages (total: ${mergedMessages.length})`,
    );
...
async function backfillChannelFromDiscord(
  ...
  try {
    ...
    let afterCursor: string = afterDiscordMessageId;
    ...
    if (newMessages.length > 0) {
      console.log(
        `[Backfill] Channel ${channelId} fetched ${newMessages.length} new messages from Discord`,
      );
    }
  ...
  if (botId) {
    ...
    try {
      ...
      if (lastRowId !== null && messageIdForReset) {
        ...
        console.log(
          `[Reset] Recorded reset boundary for bot ${botId} in thread ${threadId} at row_id ${lastRowId}, Discord msg ${messageIdForReset}`,
        );
      }
    } catch (err) {
    ...
    try {
      ...
      if (lastRowId !== null && messageIdForReset) {
        ...
        console.log(
          `[Reset] Recorded global reset boundary for thread ${threadId} at row_id ${lastRowId}, Discord msg ${messageIdForReset}`,
        );
      }
